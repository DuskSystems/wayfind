...
Wildcard parameters allow matching one or more segments in a route.

They can occur in the middle of a route, or at the end, as a catch-all.
...
Once registered to a router, a constraint can be attached to any route via the following syntax: `/{name:constraint}` or `/{*name:constraint}`.
