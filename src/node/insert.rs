...
impl<'a, T> Node<'a, T> {
    pub fn insert(&mut self, route: &mut Route<'a>, data: NodeData<T>) -> Result<(), InsertError> {
    ...
    fn insert_static(&mut self, route: &mut Route<'a>, data: NodeData<T>, prefix: &'a [u8]) -> Result<(), InsertError> {
        let Some(child) = self
            .static_children
            .iter_mut()
