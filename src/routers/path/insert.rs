...
impl<'r, S: State> Node<'r, S> {
    ...
    /// Will return the pre-existing ID on conflict.
    pub fn insert(&mut self, route: &mut ParsedRoute, data: PathData<'r>) -> PathId {
        let id = data.id;
        ...
        if let Some(part) = route.parts.pop() {
            match part {
                ...
                Part::Wildcard {
                    name, constraint, ..
                } if route.parts.is_empty() => self.insert_end_wildcard(data, name, constraint),
                Part::Wildcard {
                    name, constraint, ..
    ...
    fn insert_static(
        ...
        if !self
            .static_children
            .iter()
            .any(|child| child.state.prefix[0] == prefix[0])
        {
            ...
            let result_id = new_child.insert(route, data);
        ...
        if common_prefix >= child.state.prefix.len() {
            let result_id = if common_prefix >= prefix.len() {
            ...
            if prefix[common_prefix..].is_empty() {
                ...
                let result_id = new_child_b.insert(route, data);
    ...
    fn insert_dynamic(
        ...
        {
            let result_id = child.insert(route, data);
            self.needs_optimization = true;
            result_id
        } else {
            let mut new_child = Node {
            ...
            let result_id = new_child.insert(route, data);
            self.dynamic_children.push(new_child);
            self.needs_optimization = true;
            result_id
        }
    }
    ...
    fn insert_wildcard(
            ...
            .find_mut(|child| child.state.name == name && child.state.constraint == constraint)
        {
            ...
            let result_id = new_child.insert(route, data);
            self.wildcard_children.push(new_child);
            self.needs_optimization = true;
            result_id
        }
    }
    ...
    fn insert_end_wildcard(
        ...
        if let Some(child) = existing {
            ...
            self.end_wildcard_children.push(new_child);
            self.needs_optimization = true;
            id
        }
    }
