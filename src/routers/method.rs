use super::authority::id::AuthorityId;
use crate::PathId;
use errors::{MethodDeleteError, MethodInsertError, MethodSearchError};
use id::MethodIdGenerator;
use std::collections::BTreeMap;

pub mod display;
pub mod errors;
pub mod id;

pub use id::MethodId;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MethodMatch<'r> {
    pub id: MethodId,
    pub method: Option<&'r str>,
}

#[derive(Clone)]
pub struct MethodRouter {
    pub map: BTreeMap<(AuthorityId, PathId), BTreeMap<String, MethodId>>,
    pub id: MethodIdGenerator,
}

impl MethodRouter {
    pub fn new() -> Self {
        Self {
            map: BTreeMap::default(),
            id: MethodIdGenerator::default(),
        }
    }

    pub fn insert(
        &mut self,
        authority_id: AuthorityId,
        path_id: PathId,
        methods: &[&str],
    ) -> Result<MethodId, MethodInsertError> {
        if methods.is_empty() {
            return Err(MethodInsertError::Empty);
        }

        let map = self.map.entry((authority_id, path_id)).or_default();

        for method in methods {
            if map.contains_key(*method) {
                return Err(MethodInsertError::Conflict);
            }
        }

        let id = self.id.next();
        for method in methods {
            map.insert((*method).to_owned(), id);
        }

        Ok(id)
    }

    pub fn search<'r>(
        &self,
        authority_id: AuthorityId,
        path_id: PathId,
        method: &'r str,
    ) -> Result<Option<MethodMatch<'r>>, MethodSearchError> {
        if let Some(method_map) = self.map.get(&(authority_id, path_id)) {
            if let Some(id) = method_map.get(method) {
                return Ok(Some(MethodMatch {
                    id: *id,
                    method: Some(method),
                }));
            }

            return Err(MethodSearchError::NotAllowed);
        }

        Ok(None)
    }

    pub fn find(
        &self,
        authority_id: AuthorityId,
        path_id: PathId,
        methods: &[&str],
    ) -> Result<MethodId, MethodDeleteError> {
        if methods.is_empty() {
            return Ok(MethodId(None));
        }

        let map = self
            .map
            .get(&(authority_id, path_id))
            .ok_or(MethodDeleteError::NotFound)?;

        for method in methods {
            if !map.contains_key(*method) {
                return Err(MethodDeleteError::Mismatch);
            }
        }

        let first_id = map.get(methods[0]).unwrap();
        for method in methods {
            match map.get(*method) {
                Some(id) if id == first_id => continue,
                _ => return Err(MethodDeleteError::Mismatch),
            }
        }

        if map
            .iter()
            .any(|(method, id)| id == first_id && !methods.contains(&method.as_str()))
        {
            return Err(MethodDeleteError::Mismatch);
        }

        Ok(*first_id)
    }

    pub fn delete(
        &mut self,
        authority_id: AuthorityId,
        path_id: PathId,
        methods: &[&str],
    ) -> Result<MethodId, MethodDeleteError> {
        // FIXME: Make this get passed in?
        let id = self.find(authority_id, path_id, methods)?;

        if let Some(map) = self.map.get_mut(&(authority_id, path_id)) {
            map.retain(|_, method_id| method_id != &id);
            if map.is_empty() {
                self.map.remove(&(authority_id, path_id));
            }
        }

        Ok(id)
    }
}
