...
use crate::errors::{EncodingError, PercentEncodingError};
use std::borrow::Cow;
...
pub fn percent_decode(input: &[u8]) -> Result<Cow<'_, [u8]>, EncodingError> {
    ...
    while i < len {
        match input[i] {
            b'%' if i + 2 >= len => {
                return Err(EncodingError::Percent(
                    PercentEncodingError::InvalidCharacter {
...
mod tests {
    ...
    fn test_percent_incomplete_percent() {
        ...
        assert_eq!(
            result,
            EncodingError::Percent(PercentEncodingError::InvalidCharacter {
                input: String::from_utf8_lossy(input).to_string(),
                position: 0,
                character: vec![b'%'],
            })
